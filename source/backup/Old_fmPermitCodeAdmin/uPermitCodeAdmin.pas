...
uses
  ...
  AdvToolBtn,ADODB,ActiveX, uSubForm, CommandArray;
...
type
  TfmPermitCodeAdmin = class(TfmASubForm)
    ...
    lb_page1: TAdvSmoothLabel;
    lb_page2: TAdvSmoothLabel;
    lb_page3: TAdvSmoothLabel;
    lb_page4: TAdvSmoothLabel;
    lb_page5: TAdvSmoothLabel;
    lb_Nextpage: TAdvSmoothLabel;
    lb_PrePage: TAdvSmoothLabel;
...
begin
  ...
  if stCode = '' then
  begin
    showmessage(stringReplace(MSGNOTNAMEERROR,'$NAME','출입승인코드',[rfReplaceAll]));
    Exit;
  end;

  if DupCheckPermitCode(stCode,stDupName) then
  begin
    showmessage(stDupName + '에서 이미 사용중인 코드입니다.');
  end;
...
procedure TfmPermitCodeAdmin.lb_NextpageClick(Sender: TObject);
begin
  inherited;
  L_nCurrentPageList := lb_page5.Tag + 1;
...
procedure TfmPermitCodeAdmin.lb_PrePageClick(Sender: TObject);
begin
  inherited;
