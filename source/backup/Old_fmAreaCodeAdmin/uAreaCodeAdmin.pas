...
begin
  ...
  begin
    showmessage(stringReplace(MSGNOTNAMEERROR,'$NAME','동코드',[rfReplaceAll]));
    Exit;
  end;
...
procedure TfmAreaCodeAdmin.FormActivate(Sender: TObject);
begin
  inherited;
  FormNameSetting;
end;
...
(*      stSql := stSql + ' Where BC_POSITION = 2 ';
      if cmb_ListDongCode.ItemIndex > 0 then
      begin
      ...
      begin
        stSql := stSql + ' AND BC_NAME Like ''%' + ed_areaname.Text + '%'' ';
      end;
...
begin
  ...
  begin
    ...
    begin
      showmessage(MSGDEVELOPERROR);
      Exit;
    end;
    nIndex := UpdateDongCodeList.IndexOf(cells[3,Row]);
    if nIndex < 0 then
    begin
      showmessage(MSGCODEEMPTYERROR);
      Exit;
    end;
